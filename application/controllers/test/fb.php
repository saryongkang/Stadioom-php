...
class Fb extends Test_REST_Controller {
    ...
    public function all_get() {
        try {
            ...
            $result = $this->sendPost("api/fb/connect", array('fbId' => '111', 'fbAccessToken' => 'bypass me.', 'fbExpires' => 0
                    ));
            ...
            $result = $this->sendPost("api/fb/connect", array('fbId' => $testUser['fbId'], 'fbAccessToken' => $testUser['fbAccessToken'], 'fbExpires' => 0));
            ...
            $result = $this->sendPost("api/fb/deauthorize", array('accessToken' => 'invalid_token'));
            $this->assertArray($result, 'error_code', 400);

            // deauthorize with valid registered fbId
            $result = $this->sendPost("api/fb/deauthorize", array('accessToken' => $accessToken));
            $this->assertEquals($result, 'OK');


            // invite 'invalid' accessToken -> failed
            $result = $this->sendPost("api/fb/invite", array('accessToken' => 'invalid_token', 'inviteeFbIds' => array('123', '456')));
            $this->assertArray($result, 'error_code', 400);

            // invite 'invalid' fbId -> invalid
            $result = $this->sendPost("api/fb/invite", array('accessToken' => $accessToken,
            ...
            $result = $this->sendPost("api/fb/invite", array('accessToken' => 'invalid.addess.token',
                'inviteeFbIds' => array($testUser['fbId'], 'invalid.fb.id', '123')
                    ));
            $this->assertArray($result, 'error_code', 400);
